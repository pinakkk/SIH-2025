  ...
  return (
    <div className="min-h-screen font-sans pb-28 bg-gray-50 text-gray-900 dark:bg-[#1f1816] dark:text-white">
      ...
      <motion.header
        ...
        transition={{ duration: 0.25 }}
        className="sticky top-0 z-40 bg-white/70 dark:bg-[#2b2320]/55 border-b border-gray-200 dark:border-[#3a2f2d] px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center"
      >
        <div className="flex items-center gap-3">
          <button
            onClick={() => setSidebarOpen(true)}
            className="w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg bg-white text-gray-700 border border-gray-200 dark:bg-[#372a28] dark:text-white dark:border-transparent hover:brightness-95 transition"
          ...
          <div>
            <h1 className="font-bold text-xl sm:text-2xl">Live Hazard Map</h1>
            <p className="text-sm sm:text-base text-gray-600 dark:text-[#d8cdc6] flex items-center">
              <MapPin size={14} className="mr-1" /> {userLocation}
            </p>
          </div>
        </div>
        <motion.button
          ...
          whileHover={{ scale: 1.05 }}
          onClick={() => window.location.reload()}
          className="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white text-gray-700 border border-gray-200 dark:bg-[#372a28] dark:text-white dark:border-transparent flex items-center justify-center transition-transform"
          ...
          {loading
            ...
            : hotspots.map((h, i) => (
                <motion.div
                  ...
                  transition={{ duration: 0.4, delay: i * 0.1 }}
                >
                  <Card className="bg-white dark:bg-gradient-to-b dark:from-[#2a1e1c] dark:to-[#1e1614] border border-gray-200 dark:border-[#3a2f2d] rounded-2xl overflow-hidden shadow-sm dark:shadow-lg">
                    <CardContent className="p-4 sm:p-5">
                      <h2 className="font-semibold text-gray-900 dark:text-white capitalize mt-1 mb-2 text-sm sm:text-base md:text-lg">
                        {h.title}
                      </h2>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-1 gap-x-3 text-xs sm:text-sm text-gray-600 dark:text-[#bfb2ac]">
                        <p>Reports: {h.reports}</p>
                        <p>Detected: {new Date(h.createdAt).toLocaleString()}</p>
                        <p>Updated: {new Date(h.updatedAt).toLocaleString()}</p>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 mt-2 mb-3">
                        <span
                          className={`w-2 h-2 rounded-full ${
                            h.isLive ? "bg-red-500 animate-pulse" : "bg-gray-400"
                          }`}
                        ></span>
                        <span
                          className={`text-xs font-medium ${
                            h.isLive ? "text-red-500" : "text-gray-500"
                          }`}
                        >
                          {h.isLive ? "Live" : "Not Live"}
                        </span>
                      ...
                      {isLoaded &&
                        h.location &&
                        h.location.length === 2 && (
                          <div className="w-full h-40 sm:h-52 md:h-64 lg:h-72 rounded-xl overflow-hidden mb-4 border border-gray-200 dark:border-[#3a2f2d]">
                            <GoogleMap
                              mapContainerStyle={{ width: "100%", height: "100%" }}
                              center={{
                                lat: Number(h.location[1]),
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 mt-3">
                        <motion.button
                          whileTap={{ scale: 0.97 }}
                          className="flex-1 flex items-center justify-center bg-indigo-500 text-white hover:bg-fuchsia-500 font-semibold py-2.5 rounded-full hover:scale-[1.02] transition-transform border border-gray-200 dark:border-transparent"
                          onClick={() =>
                            navigate(`/dashboard/hazard/${h.id}/details`)
                          }
                        >
                        ...
                        <motion.button
                          whileTap={{ scale: 0.97 }}
                          className="flex-1 flex items-center justify-center bg-gray-100 text-gray-900 font-semibold py-2.5 rounded-full hover:bg-gray-200 transition-colors border border-gray-200 dark:bg-[#2b2320] dark:text-white dark:border-[#3a2f2d] dark:hover:bg-[#3b3230]"
                          onClick={() => openInMaps(h)} // ✅ Directly open Google Maps
                        >
                          View on Map
                        </motion.button>
...
export default LiveHazardMapPage;
