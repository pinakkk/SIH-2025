...
export function CommunityPage() {
  ...
  return (
    <div className="bg-[#1f1816] min-h-screen text-white font-sans pb-28">
      <motion.header
        animate={{
          boxShadow: scrolled ? "0 8px 20px rgba(0,0,0,0.6)" : "0 2px 6px rgba(0,0,0,0.25)",
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setProfileSidebarOpen(true)} className="flex items-center">
              <img src={user?.photoURL || "https://i.pravatar.cc/40"} alt="profile" width={44} height={44} className="rounded-full object-cover border-2 border-[#2f2523]" />
            </button>
          </div>
        </div>
      </motion.header>
      ...
      <div className="max-w-4xl mx-auto px-5 py-6 space-y-8">
        ...
        <div>
          <div className="flex justify-between items-center mb-5">
            ...
            <button onClick={() => navigate(ROUTES.GROUPS)} className="text-sm text-orange-400 flex items-center hover:text-orange-300 transition-colors">View All <ArrowRight size={14} className="ml-1" /></button>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-4 -mx-5 px-5 scrollbar-hide">
            {trendingGroups.length === 0 ? (
                <>{[1, 2, 3].map(idx => ( <div key={idx} className="min-w-[220px] flex-shrink-0"><div className="bg-[#29201e] rounded-xl p-6 border border-[#3a2f2d] h-[220px] flex flex-col items-center"><SkeletonBlock className="h-18 w-18 rounded-full mb-5" /><SkeletonBlock className="h-4 w-28 mb-2" /><SkeletonBlock className="h-3 w-20 mb-6" /><SkeletonBlock className="h-10 w-full rounded-lg" /></div></div>))}</>
            ) : (
                    ...
                    <motion.div key={group.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: idx * 0.1 }} className="min-w-[220px] flex-shrink-0">
                        <Card className="bg-gradient-to-b from-[#29201e] to-[#1d1514] border-2 border-[#3a2f2d] rounded-xl hover:shadow-[0_8px_25px_rgba(0,0,0,0.5)] hover:border-[#4a403d] transition-all h-[220px]">
                            <CardContent className="pt-3 pb-6 px-6 flex flex-col items-center justify-between h-full">
                                <div className="flex flex-col items-center space-y-4"><div className="h-20 w-20 rounded-full bg-[#1a1614] overflow-hidden border-4 border-orange-400/40 shadow-xl ring-2 ring-[#1d1514] flex items-center justify-center mt-0"><img src={group.avatar} alt={group.name} className="w-full h-full object-cover rounded-full" loading="lazy" /></div><div className="text-center space-y-1"><h3 className="font-semibold text-white text-sm leading-tight line-clamp-2">{group.name}</h3><p className="text-xs text-[#bfb2ac] font-medium">{group.members.toLocaleString()} members</p></div></div>
                                <button onClick={() => handleJoinGroup(group.id)} className={`w-full py-3 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 transition-all shadow-md mt-2 ${group.isJoined ? "bg-[#3a2f2d] text-[#d8cdc6] hover:bg-[#4a403d] border border-[#4a403d]" : "bg-gradient-to-r from-orange-500 to-red-500 text-white hover:brightness-110 hover:shadow-lg"}`}>{group.isJoined ? (<><Icon icon="mdi:check" width={16} height={16} />Joined</>) : (<><UserPlus size={16} />Join Group</>)}</button>
                            </CardContent>
                        </Card>
                    </motion.div>
                ))
        ...
        <div>
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => navigate('/news-and-updates')} className="text-sm text-red-400 flex items-center hover:text-red-300 transition-colors">View All <ArrowRight size={14} className="ml-1" /></button>
          </div>
          <div className="grid grid-cols-2 gap-6">
            {newsLoading ? (
                <>{[1, 2].map(idx => (<div key={idx} className="bg-[#1E1614] rounded-xl border border-[#3a2f2d] overflow-hidden shadow-lg h-[290px]"><div className="h-28 w-full"><SkeletonBlock className="h-full w-full" /></div><div className="p-3 space-y-2"><div className="flex justify-center"><SkeletonBlock className="h-6 w-24 rounded-full" /></div><SkeletonBlock className="h-5 w-full" /><SkeletonBlock className="h-5 w-3/4" /><SkeletonBlock className="h-4 w-1/2 mx-auto" /><div className="pt-2 border-t border-[#3a2f2d]"><SkeletonBlock className="h-4 w-20" /></div></div></div>))}</>
            ) : (
                newsItems.length > 0 ? newsItems.slice(0, 2).map((item, idx) => (
                    <motion.div key={item.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.2 + idx * 0.1 }} whileTap={{ scale: 0.98 }} className="cursor-pointer">
                        <Card className="bg-[#1E1614] border border-[#3a2f2d] rounded-xl hover:border-[#4a403d] hover:shadow-[0_8px_25px_rgba(0,0,0,0.5)] transition-all shadow-lg overflow-hidden h-[290px] flex flex-col">
                            <div className="h-28 w-full bg-[#1a1614] overflow-hidden flex-shrink-0">{item.urlToImage ? (<img src={item.urlToImage} alt={item.title} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" loading="lazy" onError={(e) => { const target = e.target as HTMLImageElement; target.style.display = 'none'; target.parentElement?.classList.add('bg-gradient-to-br', 'from-[#2a1f1d]', 'to-[#1a1614]'); }} />) : (<div className="w-full h-full bg-gradient-to-br from-[#2a1f1d] to-[#1a1614] flex items-center justify-center"><Icon icon="mdi:image-off" className="text-[#4a403d]" width={32} height={32} /></div>)}</div>
                            <div className="p-3 flex-1 flex flex-col justify-between"><div className="space-y-1"><div className="flex items-center justify-center mb-1"><span className="inline-block bg-red-500 text-white text-xs font-semibold px-2.5 py-1 rounded-full">{item.source || 'News Source'}</span></div><h3 className="font-bold text-white text-sm leading-tight line-clamp-2 hover:text-orange-100 transition-colors text-center">{item.title}</h3><p className="text-[#bfb2ac] text-xs text-center">{new Date(item.date).toLocaleDateString("en-US", { month: 'short', day: 'numeric', year: 'numeric' })}</p></div><div className="pt-0.5 border-t border-[#3a2f2d]"><button onClick={() => item.url.startsWith("http") ? window.open(item.url, "_blank") : navigate('/news-and-updates')} className="inline-flex items-center gap-2 text-orange-400 hover:text-orange-300 text-xs font-semibold transition-colors">Read More <Icon icon="mdi:arrow-right" width={14} height={14} /></button></div></div>
                        </Card>
                    </motion.div>
                )) : (<div className="col-span-2 text-center py-12"><Icon icon="mdi:newspaper-variant-outline" className="w-16 h-16 mx-auto mb-4 text-gray-500" /><h3 className="text-lg font-medium text-gray-300 mb-2">No news updates available</h3><p className="text-sm text-gray-400">Check back later for disaster and weather related news</p></div>)
            )}
          </div>
        </div>

        {/* Divider / Heading for Verified Posts */}
        <div className="border-b border-[#3a2f2d] pb-2">
          <h2 className="text-lg font-semibold text-orange-400 text-center">Posts from Verified Officials</h2>
        </div>
        ...
        {loading ? (
            ...
            posts.filter((p) => p.verified).map((post, i) => (
                <motion.div key={post.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: i * 0.1 }}>
                    <Card className="bg-gradient-to-b from-[#29201e] to-[#1d1514] border border-[#3a2f2d] rounded-2xl mb-5 shadow-lg">
                        <CardContent className="p-4">
                        <div className="flex justify-between items-center mb-3"><div className="flex items-center gap-3"><img src={post.avatar} alt={post.user} width={40} height={40} className="rounded-full border-2 border-[#3a2f2d]" /><div><div className="flex items-center gap-1"><p className="font-medium">{post.user}</p>{post.verified && (<span className="text-blue-400"><Icon icon="mdi:check-decagram" className="w-4 h-4" /></span>)}</div><p className="text-xs text-orange-400">{post.status}</p></div></div><span className="text-xs text-gray-400">{post.time}</span></div>
                        <p className="text-sm mb-3 text-[#e0d6d0]">{post.caption}</p>
                        {post.image && ( <div className="w-full h-44 bg-[#131212] rounded-xl overflow-hidden mb-4 border border-[#3a2f2d]"><img src={post.image} alt="Post media" className="w-full h-full object-cover" loading="lazy" /></div> )}
                        <div className="flex items-center justify-between mb-2"><p className="text-xs text-gray-400 flex items-center"><MapPin size={14} className="mr-1" />{post.location}</p><div className="flex gap-3 text-xs text-gray-400"><span>{post.likes} likes</span><span>{post.comments} comments</span></div></div>
                        <div className="border-t border-[#3a2f2d] pt-3 mt-1 flex gap-2"><button onClick={() => handleLikePost(post.id)} className="flex-1 py-2 rounded-xl bg-[#2e2a28] text-white font-medium text-sm border border-[#3a2f2d] hover:bg-[#403633] transition flex items-center justify-center gap-1.5"><Heart size={16} />Like</button><button className="flex-1 py-2 rounded-xl bg-[#2e2a28] text-white font-medium text-sm border border-[#3a2f2d] hover:bg-[#403633] transition flex items-center justify-center gap-1.5"><MessageCircle size={16} />Comment</button></div>
                        </CardContent>
                    </Card>
                </motion.div>
            ))
