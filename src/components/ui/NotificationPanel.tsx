...
const getNotificationIcon = (type: Notification['type'], category?: Notification['category']) => {
  // Use colors that work in light and dark (darker on light, lighter on dark)
  switch (type) {
    case 'emergency':
    ...
    default:
      if (category === 'weather') {
        return <Icon icon="mdi:weather-cloudy" className="text-blue-600 dark:text-blue-400 text-xl" />;
      }
      if (category === 'safety') {
        return <Icon icon="mdi:shield-alert" className="text-orange-600 dark:text-orange-400 text-xl" />;
      }
      if (category === 'community') {
        return <Icon icon="mdi:account-group" className="text-purple-600 dark:text-purple-400 text-xl" />;
      }
...
const getPriorityColor = (priority: Notification['priority']) => {
  // keep left border color + subtle background overlay; works with both light/dark
  switch (priority) {
    case 'critical':
...
const NotificationItem: React.FC<NotificationItemProps> = ({
  ...
  return (
    <motion.div
      ...
      className={isDeleting ? "opacity-50" : ""}
    >
      {/* light bg + dark:bg fallback, left border indicates priority */}
      <div
        className={`rounded-xl border ${getPriorityColor(notification.priority)} ${notification.isRead ? 'ring-0' : 'ring-1 ring-orange-100 dark:ring-orange-900/20'} transition-colors`}
        >
        <div className="p-4">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className={`font-medium text-sm leading-relaxed truncate ${notification.isRead ? 'text-gray-600 dark:text-gray-400' : 'text-gray-900 dark:text-white'}`}>
                      {notification.title}
                    </h3>
                    {!notification.isRead && (
                      <div className="w-2 h-2 bg-orange-500 rounded-full flex-shrink-0 animate-pulse" />
                  ...
                  <p className="text-sm text-gray-700 dark:text-[#d8cdc6] leading-relaxed line-clamp-3 mb-3">
                    {notification.message}
                  </p>
                  ...
                  <div className="flex items-center justify-between gap-2">
                    {notification.location && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-600 dark:text-gray-400 truncate max-w-[140px]">{notification.location}</span>
                      </div>
                    )}
                ...
                <button
                  onClick={() => setShowActions(!showActions)}
                  className="text-gray-500 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors p-2 flex-shrink-0 rounded-lg hover:bg-gray-100 dark:hover:bg-[#2b2b2b]"
          ...
          <AnimatePresence>
            {showActions && (
              <motion.div
                ...
                animate={{ opacity: 1, height: 'auto' }}
                exit={{ opacity: 0, height: 0 }}
                className="mt-3 pt-3 border-t border-gray-100 dark:border-[#3a2f2d] flex gap-2"
              >
                <button
                  onClick={() => notification.isRead ? onMarkAsUnread(notification.id) : onMarkAsRead(notification.id)}
                  className="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:text-gray-900 hover:dark:text-white bg-gray-50 dark:bg-[#2a2a2a] hover:bg-gray-100 dark:hover:bg-[#3a3330] rounded-lg transition-colors"
                >
                  {notification.isRead ? <EyeOff size={12} /> : <Eye size={12} />}
                  {notification.isRead ? 'Mark Unread' : 'Mark Read'}
                </button>
                <button
                  onClick={handleDelete}
                  disabled={isDeleting}
                  className="flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:text-red-700 bg-gray-50 dark:bg-[#2a2a2a] hover:bg-gray-100 dark:hover:bg-[#3a3330] rounded-lg transition-colors disabled:opacity-50"
                >
                  <Trash2 size={12} className={isDeleting ? "animate-spin" : ""} />
                  {isDeleting ? "Deleting..." : "Delete"}
                </button>
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            exit={{ opacity: 0, x: '100%' }}
            transition={{ type: 'spring', damping: 25, stiffness: 200 }}
            className="fixed top-0 right-0 h-full w-full max-w-sm sm:max-w-md z-50 flex flex-col
                       bg-white text-gray-900 border-l border-gray-200
                       dark:bg-gradient-to-b dark:from-[#2b2320] dark:to-[#1f1816] dark:text-white dark:border-[#3a2f2d]"
          >
            {/* Header */}
            <div className="flex-shrink-0 bg-white/90 dark:bg-gradient-to-r dark:from-[#2b2320]/95 dark:to-[#1f1816]/95 border-b border-gray-200 dark:border-[#3a2f2d] p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 sm:gap-4">
                  <Bell size={20} className="text-orange-500" />
                  <h2 className="font-semibold text-lg sm:text-xl text-gray-900 dark:text-white">Notifications</h2>
                  {unreadCount > 0 && (
                    <span className="bg-orange-500 text-white text-sm font-medium px-2.5 py-1 rounded-full min-w-[24px] text-center">
                      {unreadCount > 99 ? '99+' : unreadCount}
                    </span>
                ...
                <div className="flex items-center gap-2">
                  {notifications.length > 0 && (
                    <motion.button
                      onClick={handleClearAll}
                      className="flex items-center gap-1 px-2.5 py-1.5 text-sm text-gray-700 dark:text-gray-200 hover:text-white bg-gray-100 dark:bg-[#3a2f2d] hover:bg-gray-200 dark:hover:bg-[#4a3e3c] rounded-lg transition-colors"
                      whileTap={{ scale: 0.95 }}
                      disabled={isClearingAll}
                      title="Clear all notifications"
                    >
                  ...
                  <button
                    onClick={onClose}
                    className="p-2.5 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-[#3a3330] rounded-xl transition-colors"
            ...
            {/* Content */}
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 sm:px-6 py-4 bg-white dark:bg-transparent">
              {notifications.length === 0 ? (
                <motion.div
                  key="empty-state"
                  initial={{ opacity: 0 }}
                  ...
                  className="text-center py-12 sm:py-16"
                >
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gray-100 dark:bg-[#3a2f2d] rounded-full flex items-center justify-center mx-auto mb-5 sm:mb-6">
                    <Bell size={24} className="text-gray-500 dark:text-gray-300 sm:w-7 sm:h-7" />
                  </div>
                  <h3 className="font-medium text-gray-900 dark:text-white mb-3 text-base sm:text-lg">No notifications</h3>
                  <p className="text-gray-600 dark:text-gray-300 text-sm sm:text-base max-w-xs mx-auto leading-relaxed">
                    You're all caught up! New notifications will appear here.
                  </p>
                </motion.div>
              ) : (
                ...
                >
                  ...
                  {unreadNotifications.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-gray-900 dark:text-white text-sm sm:text-base mb-4 flex items-center gap-3 sticky top-0 bg-white dark:bg-gradient-to-b dark:from-[#2b2320] dark:to-[#1f1816] py-2 z-10">
                        <div className="w-2 h-2 bg-orange-500 rounded-full animate-pulse" />
                        New ({unreadNotifications.length})
                        {unreadNotifications.length > 0 && onMarkAllAsRead && (
                          <button
                            onClick={onMarkAllAsRead}
                            className="ml-auto text-xs text-orange-500 hover:text-orange-400 transition-colors"
                          >
                            Mark all as read
                          </button>
                        )}
                  ...
                  {readNotifications.length > 0 && (
                    <div>
                      <h3 className="font-semibold text-gray-600 dark:text-gray-300 text-sm sm:text-base mb-4 sticky top-0 bg-white dark:bg-gradient-to-b dark:from-[#2b2320] dark:to-[#1f1816] py-2 z-10">
                        Earlier
                      </h3>
                      <div className="space-y-4">
                        <AnimatePresence initial={false}>
...
export const NotificationButton: React.FC<NotificationButtonProps> = ({
  ...
  // Theme-aware button (light: white bg, dark: dark bg)
  return (
    <button
      onClick={onClick}
      className={`relative w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-transform
                  bg-white text-gray-700 border border-gray-200
                  dark:bg-[#372a28] dark:text-white dark:border-transparent ${className}`}
    ...
    >
      ...
      {unreadCount > 0 && (
        <motion.span
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          className="absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 w-4 h-4 sm:w-5 sm:h-5 bg-orange-500 text-white text-xs font-medium rounded-full flex items-center justify-center ring-1 sm:ring-2 ring-white dark:ring-[#372a28]"
        >
          {unreadCount > 99 ? '99+' : unreadCount > 9 ? '9+' : unreadCount}
        </motion.span>
      )}
