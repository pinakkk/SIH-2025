...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { id: "community", label: "Community", icon: <Users size={20} />, route: ROUTES.COMMUNITY },
    { id: "emergency", label: "Emergency Mode", icon: <ShieldAlert size={20} />, route: ROUTES.EMERGENCY_MODE },
    { id: "evac", label: "Evacuation Centres", icon: <Icon icon="mdi:home-group" className="text-current text-lg" />, route: ROUTES.EVACUATION_CENTERS },
    { id: "ai", label: "Get info with our AI", icon: <Icon icon="mdi:robot-outline" className="text-current text-lg" />, route: ROUTES.CHATBOT },
    { id: "help", label: "Help & Support", icon: <HelpCircle size={20} />, route: ROUTES.SUPPORT },
    { id: "news", label: "News & Updates", icon: <Newspaper size={20} />, route: ROUTES.NEWS_AND_UPDATES },
  ];
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.25 }}
            className="fixed inset-0 bg-black/40 dark:bg-black/50 z-[100]"
            onClick={onClose}
          />
          ...
          <motion.aside
            ...
            transition={{ type: "spring", stiffness: 120, damping: 20 }}
            className={
              // light mode: white bg; dark mode: gradient dark bg
              "fixed top-0 left-0 bottom-0 w-72 z-[110] flex flex-col " +
              "bg-white text-gray-900 border-r border-gray-200 shadow-2xl " +
              "dark:bg-gradient-to-b dark:from-[#2b2320] dark:to-[#1a1412] dark:text-white dark:border-[#3a2f2d]"
            }
          ...
          >
            ...
            >
              ...
              <div className="flex justify-between items-start mb-6 p-5">
                <div className="flex flex-col items-start">
                  <img src={AppLogo} alt="App Logo" className="w-12 h-12 mb-2" />
                  <h2 className="text-lg font-bold text-gray-900 dark:text-white">Rescue Saathi</h2>
                  <p className="text-xs text-gray-600 dark:text-[#d8cdc6]">
                    India’s First Social App For Disaster Safety
                  </p>
                </div>
                ...
                <button
                  onClick={onClose}
                  className="w-9 h-9 rounded-full bg-gray-100 text-gray-700 dark:bg-[#372a28] dark:text-white flex items-center justify-center hover:brightness-95 transition"
                  ...
                  return (
                    <motion.button
                      ...
                      className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl transition-all text-sm font-medium ${
                        isActive
                          ? // Active: orange gradient. Use dark:text-white so it reads on dark.
                            "bg-gradient-to-b from-orange-400 to-orange-600 text-black font-semibold shadow-lg dark:text-white"
                          : // Inactive: readable both modes
                            "bg-transparent text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-[#3a2f2d]"
                      }`}
