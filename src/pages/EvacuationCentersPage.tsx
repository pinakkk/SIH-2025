  ...
  return (
    <div className="bg-[#1f1816] min-h-screen text-white font-sans pb-20">
      ...
      <motion.header
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4 }}
        className="sticky top-0 z-40 bg-[#2b2320]/85 backdrop-blur-md border-b border-[#3a2f2d] px-5 md:px-8 lg:px-12 py-4"
      >
        <div className="flex items-center justify-between mb-4 max-w-5xl mx-auto">
          ...
          <h1 className="text-lg md:text-xl font-semibold">Evacuation Centers</h1>
          <div className="w-10" />
        </div>
        ...
        <div className="relative max-w-5xl mx-auto">
          <input
            ...
            value={searchQuery}
            onChange={(e) => handleSearch(e.target.value)}
            className="w-full px-4 py-2.5 pl-11 rounded-xl bg-[#372a28] border border-[#3a2f2d] text-white placeholder-[#bfb2ac] focus:outline-none focus:border-orange-500 transition-all text-sm md:text-base"
          />
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#bfb2ac] w-5 h-5" />
        </div>
      </motion.header>
      ...
      {/* View Toggle + Filters */}
      <div className="px-5 md:px-8 lg:px-12 py-4 max-w-5xl mx-auto">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
          {/* Toggle */}
          <div className="flex bg-[#2a2a2a] p-1 rounded-lg self-start">
            <button
              onClick={() => setActiveView('list')}
              className={`px-4 py-1.5 rounded-md transition-all ${
                activeView === 'list' ? 'bg-orange-500 text-white' : 'text-[#bfb2ac] hover:text-white'
              }`}
            >
              <Icon icon="solar:list-outline" className="text-xl" />
            </button>
            <button
              onClick={() => setActiveView('map')}
              className={`px-4 py-1.5 rounded-md transition-all ${
                activeView === 'map' ? 'bg-orange-500 text-white' : 'text-[#bfb2ac] hover:text-white'
              }`}
            >
              <Icon icon="solar:map-outline" className="text-xl" />
            </button>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => setShowFilters(!showFilters)}
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#2a2a2a] text-[#bfb2ac] hover:text-white transition-all self-start"
          >
            <Filter size={18} />
            <span>Filters</span>
          </motion.button>
      ...
      {/* Shelter List */}
      <div className="px-5 md:px-8 lg:px-12 max-w-5xl mx-auto">
        {loading ? (
          <div className="space-y-4">
            {Array(3).fill(0).map((_, i) => (
              <SkeletonBlock key={i} className="h-48" />
          ...
          >
            {filteredShelters.map((shelter) => (
              <motion.div
                ...
                className="bg-[#2a2a2a] border border-[#3a2f2d] rounded-2xl p-4 shadow-lg flex flex-col"
              >
                {/* Header */}
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-semibold text-base md:text-lg">{shelter.name}</h3>
                    <p className="text-xs md:text-sm text-[#bfb2ac] flex items-center gap-1">
                      <MapPin size={14} />
                      {shelter.address.locality}, {shelter.address.district}
                    </p>
                  </div>
                  <button
                    onClick={() => handleBookmark(shelter.id)}
                    className={`p-2 rounded-full ${
                      shelter.isBookmarked ? 'bg-orange-500/20 text-orange-400' : 'bg-[#372a28] text-[#bfb2ac]'
                    }`}
                  >
                    <Bookmark size={18} className={shelter.isBookmarked ? 'fill-orange-400' : ''} />
                  </button>
                ...
                <div className="flex justify-between items-center mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs md:text-sm font-medium">{shelter.status}</span>
                  </div>
                  <div className="text-xs md:text-sm">
                    <span className="text-orange-400 font-semibold">{shelter.capacity.available}</span>
                    <span className="text-[#bfb2ac]">/{shelter.capacity.total} beds</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mt-auto">
                  ...
                  <div className="text-xs md:text-sm text-[#bfb2ac]">{shelter.distance} km away</div>
                </div>
              </motion.div>
            ))}
          </motion.div>
